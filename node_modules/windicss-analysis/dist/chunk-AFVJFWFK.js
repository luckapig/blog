...
async function tryPort(start = 4e3) {
...
function matchRule(info, rules = defaultRules) {
  if (!info)
    return;
...
async function runAnalysis(userOptions = {}, options = {}) {
  ...
  const utils = (_a = options.utils) != null ? _a : _pluginutils.createUtils.call(void 0, userOptions, { name: NAME });
  await utils.init();
  const root = utils.options.root;
  const files = [];
  const allcodes = [];
  for (const filepath of await utils.getFiles()) {
    let code = await _fs.promises.readFile(filepath, "utf-8");
  ...
  const _files = {};
  const _groups = _declass.declass.call(void 0, allcodes.map(({ code }) => code).join("\n"));
  ...
  const packageJsonPath = _path.join.call(void 0, root, "package.json");
  if (_fs.existsSync.call(void 0, packageJsonPath)) {
    const { name, version: version2 } = JSON.parse(await _fs.promises.readFile(packageJsonPath, "utf-8"));
    result.name = name;
...
var analysisReturn;
function ApiMiddleware(windicssOptions = {}, analysisOptions = {}) {
...
async function startServer(options = {}) {
  ...
  app.use(_sirv2.default.call(void 0, _path.resolve.call(void 0, __dirname, "../dist/app"), { dev: true, single: true }));
  _http2.default.createServer(app).listen(port);
  if (open)
    (0, import_open.default)(url);
  console.log(`${import_chalk.cyan`Windi Analyser`} v${version} started at ${(0, import_chalk.yellow)(url)}`);
  return app;
}
...
async function generateBuild(options) {
  const { result } = await runAnalysis(__spreadProps(__spreadValues({}, options.windiOptions), {
    root: options.root
...
`;
  await _fsextra2.default.ensureDir(options.outDir);
  await _fsextra2.default.copy(_path.resolve.call(void 0, __dirname, "../dist/app"), options.outDir);
  let indexHTML = await _fsextra2.default.readFile(_path.resolve.call(void 0, __dirname, "../dist/app/index.html"), "utf-8");
  indexHTML = indexHTML.replace("<head>", `<head><script>${headScript}<\/script>`);
  await _fsextra2.default.writeFile(_path.resolve.call(void 0, options.outDir, "index.html"), indexHTML, "utf-8");
}
...
exports.__spreadValues = __spreadValues; exports.__spreadProps = __spreadProps; exports.uniq = uniq; exports.countElement = countElement; exports.timeout = timeout; exports.tryPort = tryPort; exports.getFileExt = getFileExt; exports.toArray = toArray; exports.registerRegExpToJSON = registerRegExpToJSON; exports.defaultRules = defaultRules; exports.matchRule = matchRule; exports.runAnalysis = runAnalysis; exports.ApiMiddleware = ApiMiddleware; exports.version = version; exports.startServer = startServer; exports.generateBuild = generateBuild;
/**
 * filesize
 *
