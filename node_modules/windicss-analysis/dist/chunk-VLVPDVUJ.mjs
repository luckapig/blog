...
async function runAnalysis(userOptions = {}, options = {}) {
  ...
  if (existsSync(packageJsonPath)) {
    const { name, version: version2 } = JSON.parse(await fs.readFile(packageJsonPath, "utf-8"));
    result.name = name;
...
function ApiMiddleware(windicssOptions = {}, analysisOptions = {}) {
  return async (req, res, next) => {
    ...
    if (path === "/report.json") {
      ...
      try {
        res.write(await fs2.readFile(filepath, "utf-8"));
        return res.end();
      } catch (e) {
        console.error(e);
        res.statusCode = 500;
...
async function startServer(options = {}) {
  ...
  app.use(sirv(resolve(__dirname, "../dist/app"), { dev: true, single: true }));
...
`;
  await fs3.ensureDir(options.outDir);
  await fs3.copy(resolve2(__dirname, "../dist/app"), options.outDir);
  let indexHTML = await fs3.readFile(resolve2(__dirname, "../dist/app/index.html"), "utf-8");
  indexHTML = indexHTML.replace("<head>", `<head><script>${headScript}<\/script>`);
  await fs3.writeFile(resolve2(options.outDir, "index.html"), indexHTML, "utf-8");
}
